...
func getGrayAll() [][]grayCase {
	fpath := "/home/shaoyu/Downloads/Gray_1_study_global_include_all_CLEAN_CASE.csv"
	b, err := os.ReadFile(fpath)
	if err != nil {
		panic(err)
	}
	rows, err := csv.NewReader(bytes.NewBuffer(b)).ReadAll()
	if err != nil {
		panic(err)
	}
	...
	slices.SortFunc(data, func(a, b []grayCase) int { return cmp.Compare(a[0].uID, b[0].uID) })
	return data
}
...
func TestGray(t *testing.T) {
	...
	for _, study := range data {
		var tt tTest
		// p-value based test.
		// from table 5 many labs 2
		tt.pUsed = min(38, len(study))
		tt.pUsed = min(9999, len(study))
		...
		// e-value based test.
		tt.stopT = -1
		// designSaviT(deltaMin=0.769, beta=0.2, alpha=0.05, testType="twoSample")
		nPlan := min(45*2, len(study))
		nPlan = min(9999, len(study))
		for n := 1; n <= nPlan; n++ {
			tt.eUsed = n
			dataN := study[:n]
			x, y := splitGray(dataN)
	...
	// t.Logf("pRejects %d/%d pUsed %d, eRejects %d/%d eUsed %d", pRejects, len(tTests), pUsed, eRejects, len(tTests), eUsed)
}
...
func TestGetNPlan(t *testing.T) {
	...
	tests := []struct {
		...
		{alpha: 0.05, beta: 0.2, deltaMin: 0.51765, nPlan: NPlan{NPlan: 94, Mean: 56}},
	}
	for i, test := range tests {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			return
			...
			if nPlan != test.nPlan {
				t.Errorf("geNPlan(%f, %f, %f): got %v want %v", test.alpha, test.beta, test.deltaMin, nPlan, test.nPlan)
			}
		})
	}
}
